...
/** A menu link: site paths through the locale-aware Link, full URLs as plain anchors. */
function MenuLink({item, onClick}: {item: NavItem; onClick?: () => void}) {
  if (/^https?:\/\//i.test(item.href)) {
    return (
      <a href={item.href} target={item.target} rel={item.target === '_blank' ? 'noopener noreferrer' : undefined} onClick={onClick}>
        {item.label}
      </a>
    );
  }
  return (
    <Link href={item.href} target={item.target} onClick={onClick}>
      {item.label}
    </Link>
  );
}

function MenuItems({items, onClick}: {items: NavItem[]; onClick?: () => void}) {
  return (
    <>
      {items.map((item) => (
        <li key={`${item.href}-${item.label}`} className={item.children.length > 0 ? 'has-dropdown' : undefined}>
...
/** `items` = the header menu from the CMS; without it the site's built-in links show. */
export default function HeaderClient({items}: {items?: NavItem[] | null}) {
  ...
  ].map((item) => ({...item, target: '_self' as const, children: []}));
            ...
            <div className="offcanvas__content">
              ...
              <div className="mobile-menu fix mean-container">
                <nav className="mean-nav">
                  <ul>
                    <MenuItems items={nav} onClick={closeMenu} />
                  </ul>
                </nav>
              </div>
